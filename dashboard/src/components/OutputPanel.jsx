...
const TABS = [
    { key: "spec", label: "Spec" }, { key: "blueprint", label: "Blueprint" },
    { key: "tasks", label: "Tasks" }, { key: "code", label: "Code" }, { key: "final", label: "Result" },
];
    ...
    return (
        <div className="panel output-panel">
            ...
            <div className="output-body">
                ...
                {activeTab === "blueprint" && <BlueprintView blueprint={blueprint} validation={validation} />}
                {activeTab === "tasks" && <TasksView taskQueue={taskQueue} taskStatuses={taskStatuses} />}
...
function BlueprintView({ blueprint, validation }) {
    if (!blueprint?.entities?.length) return <Empty text="Blueprint appears after Architect completes" />;
    return (
                ...
                <>
                    ...
                    {blueprint.apiEndpoints.map((ep, i) => (
                        <div key={i} className="endpoint-row">
                            <span className={`http-method m-${(ep.method || "GET").toLowerCase()}`}>{ep.method || "GET"}</span>
...
function TasksView({ taskQueue, taskStatuses }) {
    if (!taskQueue?.phases?.length) return <Empty text="Tasks appear after Planner completes" />;
        ...
        <div className="output-section">
            ...
            {taskQueue.phases.map((phase, pi) => (
                <div key={pi} className="task-group">
                    <h4 className="section-heading">Phase {phase.phaseNumber}: {phase.phaseName}</h4>
                    {phase.tasks?.map((task, ti) => {
                        const s = taskStatuses[task.taskId] || "pending";
                        return (
                            <div key={ti} className={`task-row task-row--${s}`}>
                                <span className="task-indicator">{s === "done" ? "[x]" : s === "in_progress" ? "[~]" : "[ ]"}</span>
    ...
    return (
        <div className="output-section">
            <div className="stat-grid">
                <div className="stat-card"><span className="stat-label">SANDBOX</span><span className="stat-value">{finalState.sandboxId || "--"}</span></div>
                <div className="stat-card"><span className="stat-label">FILES</span><span className="stat-value">{finalState.fileRegistryCount || 0}</span></div>
                <div className="stat-card"><span className="stat-label">COST</span><span className="stat-value">${(finalState.tokenUsage?.estimatedCost || 0).toFixed(4)}</span></div>
            </div>
