...
const mockCompactor = (state) => { nodeOrder.push("stateCompactor"); return {}; };
...
async function runTest() {
    ...
    graph.addNode("stateCompactor", mockCompactor);
    ...
    // Should be: select -> context -> code -> registry -> review -> execute -> snapshot -> compact
    //          -> select -> context -> code -> registry -> review -> execute -> snapshot -> compact
    //          -> select -> present
    ...
    assert(nodeOrder[7] === "stateCompactor", "Then stateCompactor");

    // Second iteration
    assert(nodeOrder[8] === "selectNextTask", "Second task: selectNextTask");
    assert(nodeOrder[9] === "contextBuilder", "Second task: contextBuilder");
...
runTest().catch(err => { console.error("  [ERROR]", err.message); console.error(err.stack); process.exit(1); });
