...
import { startProject, resumeProject, getRunStatus, getActiveRuns, cancelProject } from "../services/graphRunner.js";
import { broadcastToProject } from "../ws/handler.js";
import { buildGraph, createCheckpointer } from "../../src/config/graph.js";
import { getFileList, readFile, getSandboxPath } from "../../src/utils/sandboxManager.js";
...
router.post("/:id/resume", async (req, res) => {
    try {
        const { id } = req.params;
        if (getRunStatus(id)) return res.status(409).json({ error: "Project is already running" });

        const emit = (event) => broadcastToProject(id, event);
        const result = await resumeProject(id, emit);

        res.json({ projectId: result.projectId, status: "resumed", wsUrl: `/ws?projectId=${result.projectId}` });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
router.post("/:id/cancel", (req, res) => {
    ...
    res.json({ cancelled, message: cancelled ? "Project cancelled" : "No active run found" });
});
...
router.get("/:id/sandbox", async (req, res) => {
    try {
        ...
        if (!sandboxId) return res.status(404).json({ error: "No sandbox found" });

        let files = [];
        try { files = getFileList(sandboxId); } catch (e) { /* sandbox may not exist */ }

        res.json({ sandboxId, path: getSandboxPath(sandboxId), fileCount: files.length, files: files.slice(0, 100) });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
router.get("/:id/files/*", async (req, res) => {
    try {
        ...
        if (!sandboxId) return res.status(404).json({ error: "No sandbox" });
